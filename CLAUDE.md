...
- **Full-bleed pages** (a sub-page that wants to break out of the `<main className="mt-20 max-w-400 ...">` chrome) use a flow-layered pattern, not `position: fixed`. See `ClosedPage.tsx`: a `relative` section with negative margins (`-mt-20 -mb-32`) to bleed past the layout's top offset and bottom Copyright padding, a full-width `<Image className="w-full h-auto block" />` as the first child (the whole image renders at natural aspect ratio — no cropping), and an `absolute inset-0` content layer on top. Don't use `fixed inset-0` for backgrounds: it scroll-pins the bg and was rejected for this page. The bg image scrolls with the page like any other element.
